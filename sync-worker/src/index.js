...
function validEnvelope(value) {
  ...
  return (
    ...
    (value.parentRevision === null ||
      (Number.isInteger(value.parentRevision) && value.parentRevision >= 0)) &&
...
export default {
  async fetch(request, env) {
    ...
    const historyMatch = url.pathname.match(
      /^\/saves\/([a-f0-9]{64})\/history(?:\/([A-Za-z0-9_-]+))?$/,
    );
    if (historyMatch && saveIdPattern.test(historyMatch[1]) && request.method === "GET") {
      const [, saveId, versionId] = historyMatch;
      if (versionId) {
        if (!historyIdPattern.test(versionId)) {
          return jsonResponse({ error: "Invalid version ID" }, 404, headers);
        }
    ...
    if (request.method === "PUT") {
      ...
      if (envelope.version === 1 && current?.version === 2) {
        return jsonResponse(
          { error: "This app is too old to replace the current cloud save." },
          409,
          headers,
        );
      }
